...
def test_knowledge_chat(api="/chat/knowledge_base_chat"):
    url = f"{api_base_url}{api}"
    data = {
        "query": "How to ask a question to get a high-quality answer",
...
def test_search_engine_chat(api="/chat/search_engine_chat"):
    ...
    data["query"] = "What's the latest in room-temperature superconductivity?"
    ...
    for se in ["bing", "duckduckgo"]:
        ...
        if se == "bing" and not BING_SUBSCRIPTION_KEY:
            ...
            assert data["msg"] == f"To use the Bing search engine, you need to set 'BING_SUBSCRIPTION_KEY"
