...
def test_running_models(api="/llm_model/list_running_models"):
    ...
    print("\nGet a list of models currently running:")
    pprint(r.json())
    assert isinstance(r.json()["data"], list)
    assert len(r.json()["data"]) > 0


# The stop_model feature is not recommended. According to the current implementation, if it is stopped, it can only be started manually
...
def test_change_model(api="/llm_model/change_model"):
    ...
    print(f"\nTry to switch the model from {model_name} to {new_model_name}")
