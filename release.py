...
def main():
    print("Current recent Git tag:")
    latest_tag = get_latest_tag()
    print(latest_tag)

    print("Please select the part of the version number to increment (X, Y, Z):")
    increment = input().upper()

    while increment not in ['X', 'Y', 'Z']:
        print("Mistyped, please enter X, Y or Z:")
        increment = input().upper()

    new_version = update_version_number(latest_tag, increment)
    print(f"The new version number is: {new_version}")

    print("Confirm the update version number and push to the remote repository? (y/n)")
