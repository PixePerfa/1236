...
def is_possible_title(
    ...
    # If the text length is 0, it is definitely not the title
    if len(text) == 0:
    ...
    # There should be numbers in the first characters, and the default is within 5 characters
    if len(text) < 5:
        text_5 = text
    else:
        text_5 = text[:5]
    ...
    if len(docs) > 0:
        for doc in docs:
            if is_possible_title(doc.page_content):
                ...
                doc.page_content = f" is related to ({title}). {doc.page_content}"
        return docs
    else:
