...
@deprecated(
    since="0.3.0",
    message="The model startup feature will be rewritten in Langchain-Chatchat 0.3.x to support more modes and accelerated startup, and the related features in 0.2.x will be deprecated",
    removal="0.3.0")
...
def create_model_worker_app(log_level: str = "INFO", **kwargs) -> FastAPI:
    """
    The fields included in kwargs are as follows:
    host:
    port:
    ...
    For models supported by Langchain:
    ...
    For offline models:
        model_path: 'model_name_or_path', the repo-id or local path of the huggingface
        device:`LLM_DEVICE`
    """
    import fastchat.constants
    fastchat.constants.LOGDIR = LOG_PATH
    ...
    for k, v in kwargs.items():
        setattr(args, k, v)
    if worker_class := kwargs.get("langchain_model"): # Langchian supports no operation
    ...
    # Local model
        ...
        if kwargs["model_names"][0] in VLLM_MODEL_DICT and args.infer_turbo == "vllm":
            ...
            args.max_num_batched_tokens = None # The maximum number of tokens in a batch, depending on your graphics card and large model settings, too large settings will not be enough video memory
            args.max_num_seqs = 256
            args.disable_log_stats = False
            args.conv_template = None
            args.limit_worker_concurrency = 5
            args.no_register = False
            args.num_gpus = 1 # The sharding of the VLLM worker is tensor parallel, and the number of graphics cards is filled in here
            args.engine_use_ray = False
            args.disable_log_requests = False

            # 0.2.1 Parameters to be added after vllm, but not needed here
            ...
            args.gpus = "0" # The number of the GPU, which can be set to "0,1,2,3" if there are multiple GPUs.
            args.max_gpu_memory = "22GiB"
            args.num_gpus = 1 # The sharding of the model worker is model parallelism, fill in the number of graphics cards here
...
def run_controller(log_level: str = "INFO", started_event: mp. Event = None):
    ...
    def release_worker(
            model_name: str = Body(..., description="Name of the model to be released", samples=["chatglm-6b"]),
            # worker_address: str = Body(None, description="The address to be released from the model, choose one from the name", samples=[FSCHAT_CONTROLLER_address()]),
        ...
        if new_model_name in available_models:
            msg = f"The LLM model to be switched to {new_model_name} already exists"
            logger.info(msg)
        ...
        if new_model_name:
            ...
            logger.info (f"Coming to Stop LLM Model: {model_name}")
...
def parse_args() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--quiet",
        action="store_true",
        help="Reduce fastchat service log messages",
        dest="quiet",
    )
    parser.add_argument(
        "-i",
        "--lite",
        action="store_true",
        help="Run in Lite mode: LLM conversations, search engine conversations for online APIs only",
        dest="lite",
    )
    args = parser.parse_args()
    return args, parser
...
def dump_server_info(after_start=False, args=None):
    ...
    print(f"Tokenizer currently in use: {TEXT_SPLITTER_NAME}")
    ...
    if after_start:
        print("\n")
        print(f"Server-side running information:")
        if args.openai_api:
...
async def start_main_server():
    ...
    if len(sys.argv) > 1:
        ...
        logger.info (f"To view llm_api logs, go to {LOG_PATH}")
    ...
    if process_count() == 0:
        ...
        try:
            # Ensure that the task can exit normally after receiving SIGINT
            if p := processes.get("controller"):
                p.start()
                p.name = f"{p.name} ({p.pid})"
                controller_started.wait() # Wait for the controller to start
            ...
            while True:
                ...
                if isinstance(cmd, list):
                    ...
                    if cmd == "start": # Run the new model
                        logger.info (f"Ready to start a new model process: {new_model_name}")
                        process = Process(
                            target=run_model_worker,
                        ...
                        e.wait()
                        logger.info (f"Successfully started the new model process: {new_model_name}")
                    elif cmd == "stop":
                        if process := processes["model_worker"].get(model_name):
                            time.sleep(1)
                            process.terminate()
                            process.join()
                            logger.info (f"Stop Model Process: {model_name}")
                        else:
                        ...
                        if process := processes["model_worker"].pop(model_name, None):
                            ...
                            logger.info(f"Successfully started the new model process: {new_model_name}. Elapsed time: {timing}. ")
                        else:
                            logger.error(f"Model process not found: {model_name}")
...
# Example of API call after service startup:
# import openai
