    ...
    def _get_elements(self) -> List:
        def ppt2text(filepath):
            ...
            def extract_text(shape):
                ...
                if shape.shape_type == 13:  # 13 indicates pictures
                    image = Image.open(BytesIO(shape.image.blob))
                    result, _ = ocr(np.array(image))
                    if result:
                        ocr_result = [line[1] for line in result]
                        resp += "\n".join(ocr_result)
                elif shape.shape_type == 6:  # 6 indicates combination
            ...
            for slide_number, slide in enumerate(prs.slides, start=1):
                ...
                sorted_shapes = sorted(slide.shapes,
                                       key=lambda x: (x.top, x.left))  # Traverse from top to bottom, from left to right
