...
def list_file_num_docs_id_by_kb_name_and_file_name(session,
    ...
    '''
    Lists the IDs of all documents corresponding to a file in a knowledge base.
    Return form: [str, ...]
    '''
...
def list_docs_from_db(session,
    ...
    '''
    Lists all documents that correspond to a file in a knowledge base.
    Return form: [{"id": str, "metadata": dict}, ...]
    '''
...
def delete_docs_from_db(session,
    ...
    '''
    Delete all documents corresponding to a file in a knowledge base and return the deleted documents.
    Return form: [{"id": str, "metadata": dict}, ...]
    '''
...
def add_docs_to_db(session,
                   ...
                   doc_infos: List[Dict]):
    '''
    Add all document information corresponding to a file in a knowledge base to the database.
    doc_infos form: [{"id": str, "metadata": dict}, ...]
    '''
    # ! In this case, the doc_infos is None, and further investigation is required
    if doc_infos is None:
        print("The doc_infos parameter of the input server.db.repository.knowledge_file_repository.add_docs_to_db is None")
        return False
    for d in doc_infos:
        obj = FileDocModel(
            kb_name=kb_name,
...
def add_file_to_db(session,
                   ...
                   doc_infos: list[Dict] = [], # form: [{"id": str, "metadata": dict}, ...]
                   ):
    kb = session.query(KnowledgeBaseModel).filter_by(kb_name=kb_file.kb_name).first()
    if kb:
        # If the file already exists, update the file information and version number
