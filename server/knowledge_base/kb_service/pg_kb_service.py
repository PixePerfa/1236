    ...
    def do_drop_kb(self):
        with Session(PGKBService.engine) as session:
            session.execute(text(f'''
                    -- Deletes the records in the langchain_pg_embedding table that are linked to the langchain_pg_collection table
                    DELETE FROM langchain_pg_embedding
                    WHERE collection_id IN (
                      SELECT uuid FROM langchain_pg_collection WHERE name = '{self.kb_name}'
                    );
                    -- Delete the record in the langchain_pg_collection table
...
if __name__ == '__main__':
    ...
    print(pGKBService.search_docs("How to start an API service"))
