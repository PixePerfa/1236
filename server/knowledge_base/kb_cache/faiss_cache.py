...
class ThreadSafeFaiss(ThreadSafeObject):
    ...
    def save(self, path: str, create_path: bool = True):
        with self.acquire():
            ...
            logger.info (f"Vector library {self.key} saved to disk")
        return ret
    ...
    def clear(self):
        ...
        with self.acquire():
            ...
            logger.info (f"Vector library {self.key} emptied")
        return ret
...
class _FaissPool(CachePool):
    ...
    def unload_vector_store(self, kb_name: str):
        if cache := self.get(kb_name):
            self.pop(kb_name)
            logger.info (f"Successfully released vector library: {kb_name}")
...
class KBFaissPool(_FaissPool):
    def load_vector_store(
        ...
        cache = self.get((kb_name, vector_name)) # It's better to use tuples than to concatenate strings
        if cache is None:
            item = ThreadSafeFaiss((kb_name, vector_name), pool=self)
            self.set((kb_name, vector_name), item)
            with item.acquire(msg="initialization"):
                self.atomic.release()
