...
class ThreadSafeObject:
    ...
    def acquire(self, owner: str = "", msg: str = "") -> FAISS:
        ...
        try:
            ...
            if log_verbose:
                logger.info(f"{owner} Start: {self.key}. {msg}")
            yield self._obj
        finally:
            if log_verbose:
                logger.info(f"{owner} End operation: {self.key}. {msg}")
            self._lock.release()
        ...
        if not self.get(key):
            ...
            with item.acquire(msg="initialization"):
                ...
                if model == "text-embedding-ada-002":  # openai text-embedding-ada-002
                    ...
                    if 'zh' in model:
                        # for Chinese model
                        query_instruction = "Generate a representation for this sentence to be used to retrieve related articles:"
                    elif 'en' in model:
                        # for english model
