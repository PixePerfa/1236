...
def import_from_db(
    ...
    """
    If there is no change between the knowledge base and the vector base, import data from the backup database to the info.db.
    Applies when the version is upgraded, info.db the structure changes, but there is no need to re-vectorize.
    Make sure that the names of the tables in the two databases are the same, and the names of the fields to be imported are the same
    ...
    try:
        ...
        print(f"Unable to read backup database: {sqlite_path}. Error message: {e}")
        return False
...
def folder2db(
    ...
    def files2vs(kb_name: str, kb_files: List[KnowledgeFile]):
        for success, result in files2docs_in_thread(kb_files,
            ...
            if success:
                ...
                print(f"Adding {kb_name}/{filename} to the vector library with {len(docs)} documents")
    ...
    for kb_name in kb_names:
        ...
        # Clear the vector library and rebuild it from the local file
        if mode == "recreate_vs":
            kb.clear_vs()
            kb.create_kb()
        ...
        # # Do not vectorize the file content, only store the file meta information in the database
        # # Since there is a lot of information related to text segmentation in the data inventory, it is not meaningful to simply store file information, so this function is canceled.
        elif mode == "fill_info_only":
            files = list_files_from_folder(kb_name)
            kb_files = file_to_kbfile(kb_name, files)
            for kb_file in kb_files:
                add_file_to_db(kb_file)
        #print(f"{kb_name}/{kb_file.filename} added to database)
        #Based on the database Chinese list, update the vector library with local files
        elif mode == "update_in_db":
            files = kb.list_files()
            kb_files = file_to_kbfile(kb_name, files)
            files2vs(kb_name, kb_files)
            kb.save_vector_store()
        # Compare the list of files in the local directory with the database for incremental vectorization
        elif mode == "increment":
