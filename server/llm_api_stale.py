"""
Call example: python llm_api_stale.py --model-path-address THUDM/chatglm2-6b@localhost@7650 THUDM/chatglm2-6b-32k@localhost@7651
Other fastchat.server.controller/worker/openai_api_server parameters can be called according to the fastchat documentation
However, a few non-critical parameters such as --worker-address,--allowed-origins,--allowed-methods,--allowed-headers are not supported
...
# 2,LOG_PATH
# 3, The file name of the log
base_launch_sh = "nohup python3 -m fastchat.serve. {0} {1} >{2}/{3}.log 2>&1 &"

# 0 log_path
# ! The file name of the 1 log must be the same as the bash_launch_sh
...
def string_args(args, args_list):
    """Convert key from args to string"""
    args_str = ""
    for key, value in args._get_kwargs():
        # The key in the args._get_kwargs is delimited by _ and then converted to determine whether it is in the specified args list
        key = key.replace("_", "-")
        if key not in args_list:
            continue
        # In fastchat, the port and host do not have a prefix, remove the prefix
...
def launch_worker(item, args, worker_args=worker_args):
    ...
    # Split the model-path-address and analyze the parameters in the string_args
    args.model_path, args.worker_host, args.worker_port = item.split("@")
    args.worker_address = f"http://{args.worker_host}:{args.worker_port}"
    print("*" * 80)
    print (f"If it has not been started for a long time, please go to {LOG_PATH}{log_name}.log to check the logs")
...
def launch_all(args,
    ...
    print(f"Start the LLM service, please go to {LOG_PATH} to monitor the logs of each module...")
    ...
    print(f"The worker startup time varies depending on the device, it takes about 3-10 minutes, please be patient...")
    if isinstance(args.model_path_address, str):
        ...
        for idx, item in enumerate(args.model_path_address):
            print(f"Start loading {idx} model: {item}")
    ...
    print("The LLM service is started.")


if __name__ == "__main__":
    args = parser.parse_args()
    # http body must be added, otherwise InvalidSchema: No connection adapters were found
