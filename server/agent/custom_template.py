...
class CustomOutputParser(AgentOutputParser):
    ...
    def parse(self, llm_output: str) -> AgentFinish | tuple[dict[str, str], str] | AgentAction:
        ...
        if len(parts) < 2:
            return AgentFinish(
                return_values={"output": f"Failed to call the agent, the answer is the answer of the large model's own capabilities:\n\n '{llm_output}'"},
                log=llm_output,
            )
        ...
        try:
            ...
            return AgentFinish(
                return_values={"output": f"Agent call failed: '{llm_output}'"},
                log=llm_output,
            )
